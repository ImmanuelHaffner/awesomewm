...
local function check_focus(obj)
    ...
    if not client.focus or not client.focus:isvisible() then
        local c = aclient.focus.history.get(obj.screen, 0, function (c)
            if screen[c.screen] == screen[obj.screen] then
        ...
        if c then
            c:emit_signal("request::activate", "myautofocus.check_focus", {raise=false})
        end
    end
end
...
local function check_focus_tag(t)
    ...
    if client.focus and screen[client.focus.screen] ~= s then
        ...
        if c then
            c:emit_signal("request::activate", "myautofocus.check_focus_tag",
                          {raise=false})
        end
    end
end
